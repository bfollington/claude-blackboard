...
### Creating a Project-Specific Worker Image

To add project-specific dependencies (language runtimes, packages, build tools):

1. Generate a template Dockerfile:
   ```bash
   blackboard init-worker
   ```

2. Edit `Dockerfile.worker` in your project root to add your dependencies. The template includes examples for Python, Rust, Go, and Node.js projects.

3. Build and spawn workers with your custom image:
   ```bash
   blackboard spawn my-thread --build
   ```

### Dockerfile Lookup Priority

When you run `blackboard spawn --build` or `blackboard farm --build`, the CLI looks for a Dockerfile in this order:

1. `Dockerfile.worker` (project root) - Your project-specific customizations
2. `blackboard/docker/Dockerfile` (plugin directory) - The default base image

If neither exists, you'll be prompted to run `blackboard init-worker`.

### What Must Stay in the Dockerfile

Your custom Dockerfile must preserve these essentials:

- Claude Code CLI (`npm install -g @anthropic-ai/claude-code`)
- Deno + blackboard CLI installation
- Non-root `worker` user (Claude CLI security requirement)
- The entrypoint script
- Environment variables: `CLAUDE_PROJECT_DIR=/app/work` and `CLAUDE_PLUGIN_ROOT=/app`
